...
public class RealmService {
    ...
    public ResponseEntity<?> createRealm(NameDescriptionDto nameDescriptionDto) {
        ...
        if (name == null || name.isEmpty()) {
            return new ResponseEntity<>(new ExceptionResponse("InvalidRealmName"), HttpStatus.BAD_REQUEST);
        } else if (getRealmByName(name) != null) {
            return new ResponseEntity<>(new ExceptionResponse("DuplicateRealmName"), HttpStatus.BAD_REQUEST);
        }
    ...
    public ResponseEntity<?> getRealmById(String idString) throws ResponseStatusException {
        ...
        try {
            ...
            return new ResponseEntity<>(new ExceptionResponse("InvalidArgument"), HttpStatus.BAD_REQUEST);
        }
        if (id < 0) {
            return new ResponseEntity<>(new ExceptionResponse("InvalidArgument"), HttpStatus.BAD_REQUEST);
        }
        ...
        if (realm == null) {
            return new ResponseEntity<>(new ExceptionResponse("RealmNotFound"), HttpStatus.NOT_FOUND);
        }
        RealmDto realmDto = new RealmDto(realm.getId(), realm.getName(), realm.getDescription(), realm.getKey());
        return new ResponseEntity<>(realmDto, HttpStatus.CREATED);
    }
